...
def main():
    ...
    print(f'Min: {values.min() / 1000 : .2f}mJ\tMax: {values.max() / 1000 : .2f}mJ\tMean: {values.mean() / 1000 :.2f}mJ')

    #for iteration in range(n_iter + 1):
    #    if iteration % 100 == 0:
    #        print(f"{(iteration+1) / (n_iter+1) * 100 : .2f}% done")
    #    if device == 0:
    #        x = [14 * np.random.random_sample((32,8)) - 7.135]
    #    else:
    #        x = [14 * np.random.random_sample((1,8)) - 7.135]
...
def inferenz(x):
    return compiled_model(x)[output_layer]
